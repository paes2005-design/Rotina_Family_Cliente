...
function dataDaTarefa(t){if(/^\d{4}-\d{2}-\d{2}$/.test(String(t.dataExecucao||'')))return t.dataExecucao;for(const valor of [t.terminoExecutadoEm,t.inicioExecutadoEm]){const d=new Date(valor||'');if(Number.isFinite(d.getTime()))return dataLocal(d);}return '';}
...
function payloadHistorico(t,data){const h={grupoId:grupo(),perfilId:t.perfilId||perfil(),perfilNome:t.perfilNome||nome(),tarefaId:t.id,nomeTarefa:t.nome||t.nomeTarefa||'Tarefa',data,dataExecucao:data,status:t.status||'',reconciliadoEm:new Date().toISOString(),reconciliadoPor:'CLIENTE',origemReconciliacao:'tarefa-concluida-sem-historico'};CAMPOS.forEach(c=>{if(t[c]!==undefined)h[c]=t[c];});if(h.pontosOriginais===undefined)h.pontosOriginais=Number(h.pontosGanhos)||0;if(h.percentualOriginal===undefined&&h.percentualAplicado!==undefined)h.percentualOriginal=h.percentualAplicado;if(!h.revisaoStatus)h.revisaoStatus=h.justificativaAtraso?'aguardando':'sem-revisao';normalizarObjetoLocal(h);return h;}
