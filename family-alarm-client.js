...
import {chaveOcorrencia,deveDispararAgora,descreverProximaOcorrencia,momentoDaOcorrenciaAtual} from './alarm-schedule-core.js?v=3';
...
const KEY_SILENCED='rotina_family_task_alarm_silenced_v1';
const TONES={
...
function tarefaDaLinha(row){return {tarefaId:row.dataset.familyTaskId||'',tarefaGrupoId:row.dataset.familyTaskGroup||'',nomeTarefa:row.dataset.familyTaskName||'Tarefa',diaSemana:row.dataset.familyTaskDay||'',horaSugeridaInicio:row.dataset.familyTaskTime||'',horaSugeridaFim:row.dataset.familyTaskEnd||'',status:row.dataset.familyTaskStatus||''}}
...
async function notificarTarefa(a){if(!('Notification'in window)||Notification.permission!=='granted'||!('serviceWorker'in navigator))return;try{const momento=momentoDaOcorrenciaAtual(a,new Date()),hora=momento==='fim'?a.horaSugeridaFim:a.horaSugeridaInicio;const reg=await navigator.serviceWorker.ready;await reg.showNotification(`⏰ ${momento==='fim'?'Fim':'Início'} da tarefa: ${a.nomeTarefa||'Tarefa'}`,{body:`Programada para ${hora||'agora'}. Toque para abrir o Rotina Family.`,icon:'./icon-cliente-192.png',badge:'./icon-cliente-192.png',tag:tagNotificacao(a),renotify:true,requireInteraction:true,data:{url:'./'}})}catch{}}
...
function abrirPainel(tarefa){
  ...
  m.querySelector('#alarmToggle').onclick=()=>{const ativar=!a?.ativo,momento=m.querySelector('#alarmMoment').value;if(ativar)prepararNotificacoes();m.remove();toast(ativar?'Alarme programado.':'Alarme retirado.');gravar({...tarefa,momentos:momento==='ambos'?['inicio','fim']:[momento]},ativar,'CLIENTE').then(ok=>{if(!ok)toast('Não foi possível alterar este alarme.')})};
  m.querySelector('#alarmClose').onclick=()=>m.remove();m.onclick=e=>{if(e.target===m)m.remove()};
}

function payloadDaTarefa(tarefa,ativo,origem){const agora=new Date().toISOString();return {grupoId:grupo(),perfilId:perfil(),perfilNome:nomePerfil(),tarefaId:tarefa.tarefaId,tarefaGrupoId:tarefa.tarefaGrupoId||'',nomeTarefa:tarefa.nomeTarefa,diaSemana:tarefa.diaSemana,horaSugeridaInicio:tarefa.horaSugeridaInicio,horaSugeridaFim:tarefa.horaSugeridaFim||'',momentos:tarefa.momentos||['inicio'],ativo,origem,bloqueado:origem==='ADM'&&ativo,atualizadoEm:agora,...(ativo?{acionadoEm:agora,acionadoPor:nomePerfil()||'Cliente'}:{encerradoEm:agora,encerradoPor:nomePerfil()||'Cliente'})}}
...
function estaNaHora(a,agora){return !tarefaConcluida(a?.tarefaId)&&deveDispararAgora(a,agora,60000,silenciados[a?.tarefaId]||'')}
