...
import {agendaDaTarefa,alarmeVigente,chaveOcorrencia,dataLocal,deveDispararAgora,descreverProximaOcorrencia,momentoDaOcorrenciaAtual,semanaInicioISO} from './alarm-schedule-core.js?v=4';
...
let ctx=null,somTimer=null,relogioTimer=null,unsub=null,alarmeDisparado='',documentosRemotos=[];
let pref=ler(KEY_PREF,{tone:'classico',volume:.75});
let alarmes=ler(KEY_STATE,{});
let silenciados=ler(KEY_SILENCED,{});
...
function filtrarSemana(mapa,agora=new Date()){const g=grupo(),p=perfil();return Object.fromEntries(Object.entries(mapa||{}).filter(([,a])=>naSemanaAtual(a,agora)&&a.grupoId===g&&a.perfilId===p))}
...
function tarefaDaLinha(row){
  const tarefa={tarefaId:row.dataset.familyTaskId||'',tarefaGrupoId:row.dataset.familyTaskGroup||'',nomeTarefa:row.dataset.familyTaskName||'Tarefa',diaSemana:row.dataset.familyTaskDay||'',dataAgendada:row.dataset.familyTaskDate||'',horaSugeridaInicio:row.dataset.familyTaskTime||'',horaSugeridaFim:row.dataset.familyTaskEnd||'',status:row.dataset.familyTaskStatus||''};
...
async function prepararNotificacoes(){if(!('Notification'in window)||!('serviceWorker'in navigator))return 'indisponivel';if(Notification.permission==='default'){try{return await Notification.requestPermission()}catch{return 'default'}}return Notification.permission}
function tagNotificacao(a,agora=new Date()){return `rotina-family-${chaveOcorrencia(a,agora)}`}
async function notificarTarefa(a){if(!('Notification'in window)||Notification.permission!=='granted'||!('serviceWorker'in navigator))return;try{const momento=momentoDaOcorrenciaAtual(a,new Date(),JANELA_DISPARO_MS),hora=momento==='fim'?a.horaSugeridaFim:a.horaSugeridaInicio;const reg=await navigator.serviceWorker.ready;await reg.showNotification(`⏰ ${momento==='fim'?'Fim':'Início'} da tarefa: ${a.nomeTarefa||'Tarefa'}`,{body:`Programada para ${formatarDataBR(a.dataAgendada)} às ${hora||'agora'}. Toque para abrir o Rotina Family.`,icon:'./icon-cliente-192.png',badge:'./icon-cliente-192.png',tag:tagNotificacao(a),renotify:true,requireInteraction:true,data:{url:'./'}})}catch{}}
async function fecharNotificacao(a){if(!('serviceWorker'in navigator))return;try{const reg=await navigator.serviceWorker.ready;const lista=await reg.getNotifications({tag:tagNotificacao(a)});lista.forEach(n=>n.close())}catch{}}
...
function decorarTarefas(){
  document.querySelectorAll('tr[data-family-task-id]').forEach(row=>{
    ...
    if(!tarefa.tarefaId||!tarefa.dataAgendada||!celula)return;
    row.dataset.familyTaskDate=tarefa.dataAgendada;
    let acoes=celula.querySelector('.family-task-actions');
    if(!acoes){
      acoes=document.createElement('div');
      acoes.className='family-task-actions';
      acoes.style.cssText='display:flex;align-items:center;justify-content:center;gap:6px;flex-wrap:nowrap;min-width:max-content';
      while(celula.firstChild)acoes.appendChild(celula.firstChild);
      celula.appendChild(acoes);
    }
    let btn=celula.querySelector('.family-task-alarm-client');
    if(!btn){btn=document.createElement('button');btn.type='button';btn.className='family-task-alarm-client';btn.style.cssText='flex:0 0 36px;width:36px;height:36px;margin:0;padding:0;border:1px solid #cbd5e1;border-radius:9px;background:#fff;font-size:17px;line-height:1;cursor:pointer';btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();abrirPainel(tarefaDaLinha(row))});acoes.appendChild(btn)}
    atualizarBotao(btn,tarefa.tarefaId,tarefa.dataAgendada);
  });
}
function atualizarBotao(btn,id,dataAgendada){const a=alarmeDaTarefa(id,dataAgendada),icone=!a?.ativo?'🔕':travado(a)?'🔒':'🔔',titulo=!a?.ativo?`Programar alarme para ${formatarDataBR(dataAgendada)}`:travado(a)?'Alarme desta data programado pelo responsável':'Alarme desta data programado por você';if(btn.textContent!==icone)btn.textContent=icone;if(btn.title!==titulo)btn.title=titulo;btn.setAttribute('aria-label',titulo);btn.style.background=a?.ativo?(travado(a)?'#fee2e2':'#fff7ed'):'#fff'}
function atualizarBotoes(){document.querySelectorAll('.family-task-alarm-client').forEach(btn=>{const row=btn.closest('tr[data-family-task-id]');if(row)atualizarBotao(btn,row.dataset.familyTaskId,row.dataset.familyTaskDate)});const atual=Object.values(alarmes).find(a=>chaveAlarme(a)===alarmeDisparado);if(alarmeDisparado&&!alarmeVigente(atual,new Date()))encerrarDisparo();else verificarDisparo()}

function abrirPainel(tarefa){
  document.getElementById('familyAlarmTaskPanel')?.remove();
  tarefa={...tarefa,...agendaDaTarefa(tarefa)};
  const a=alarmeDaTarefa(tarefa.tarefaId,tarefa.dataAgendada),bloqueado=travado(a);
  ...
  m.querySelector('#alarmToggle').onclick=()=>{const ativar=!a?.ativo,momento=m.querySelector('#alarmMoment').value;if(ativar)prepararNotificacoes();m.remove();toast(ativar?'Alarme desta data programado.':'Alarme desta data retirado.');gravar({...tarefa,momentos:momento==='ambos'?['inicio','fim']:[momento]},ativar,'CLIENTE').then(ok=>{if(!ok)toast('Não foi possível alterar este alarme.')})};
  m.querySelector('#alarmClose').onclick=()=>m.remove();m.onclick=e=>{if(e.target===m)m.remove()};
}

function payloadDaTarefa(tarefa,ativo,origem){const agora=new Date().toISOString(),agenda=agendaDaTarefa(tarefa);return {grupoId:grupo(),perfilId:perfil(),perfilNome:nomePerfil(),tarefaId:tarefa.tarefaId,tarefaGrupoId:tarefa.tarefaGrupoId||'',nomeTarefa:tarefa.nomeTarefa,diaSemana:tarefa.diaSemana,horaSugeridaInicio:tarefa.horaSugeridaInicio,horaSugeridaFim:tarefa.horaSugeridaFim||'',momentos:tarefa.momentos||['inicio'],...agenda,versaoAgenda:3,ativo,origem,bloqueado:origem==='ADM'&&ativo,atualizadoEm:agora,...(ativo?{acionadoEm:agora,acionadoPor:nomePerfil()||'Cliente'}:{encerradoEm:agora,encerradoPor:nomePerfil()||'Cliente'})}}
...
function escutar(tentativa=0){if(unsub)return;const g=grupo(),p=perfil();if(!g||!p||!getApps().length){if(tentativa<120)setTimeout(()=>escutar(tentativa+1),100);return}const q=query(collection(getFirestore(getApp()),'despertadores'),where('grupoId','==',g));unsub=onSnapshot(q,s=>{const proximos=filtrarSemana(alarmes);documentosRemotos=s.docs.map(d=>({ref:d.ref,dados:{id:d.id,...d.data()}})).filter(item=>item.dados.perfilId===p);documentosRemotos.forEach(item=>{const a=item.dados;if(a.tarefaId&&naSemanaAtual(a))proximos[a.tarefaId]=proximos[a.tarefaId]?maisNovo(proximos[a.tarefaId],a):a});alarmes=proximos;salvar(KEY_STATE,alarmes);expirarAlarmesRemotos();atualizarBotoes()},()=>{atualizarBotoes()});sincronizarPendente()}
...
function estaNaHora(a,agora){return !tarefaConcluida(a)&&deveDispararAgora(a,agora,JANELA_DISPARO_MS,silenciados[a?.tarefaId]||'')}
...
function mostrarDisparo(a){alarmeDisparado=chaveAlarme(a);let o=document.getElementById('familyAlarmOverlay');if(!o){o=document.createElement('div');o.id='familyAlarmOverlay';o.style.cssText='position:fixed;inset:0;z-index:30000;background:radial-gradient(circle at top,#ef4444,#7f1d1d);color:#fff;display:flex;align-items:center;justify-content:center;padding:20px;text-align:center';document.body.appendChild(o)}const bloqueado=travado(a),momento=momentoDaOcorrenciaAtual(a,new Date(),JANELA_DISPARO_MS),hora=momento==='fim'?a.horaSugeridaFim:a.horaSugeridaInicio;o.innerHTML=`<div><div style="font-size:72px">⏰</div><div style="font-size:16px;font-weight:800;letter-spacing:.12em">${momento==='fim'?'FIM':'INÍCIO'} DA TAREFA</div><h1 style="font-size:clamp(34px,9vw,58px);margin:10px 0">${esc(a.nomeTarefa||'Tarefa')}</h1><p style="font-size:24px;font-weight:800;margin:0 0 8px">${esc(formatarDataBR(a.dataAgendada))} · ${esc(hora||'')}</p><p id="familyAlarmClock" style="font-size:18px;margin:0 0 24px"></p>${bloqueado?'<div style="background:rgba(255,255,255,.16);padding:14px 18px;border-radius:14px;font-weight:800;margin-bottom:14px">🔒 Programado pelo responsável. Você pode parar o toque, mas não retirar o alarme.</div>':''}<button id="familyAlarmStop" type="button" style="padding:15px 28px;border-radius:14px;border:0;background:#fff;color:#991b1b;font-size:18px;font-weight:900">Parar despertador</button></div>`;const clock=()=>{const e=document.getElementById('familyAlarmClock');if(e)e.textContent=new Date().toLocaleTimeString('pt-BR')};clock();relogioTimer=setInterval(clock,1000);o.querySelector('#familyAlarmStop').onclick=()=>silenciarOcorrencia(a);iniciarSom();notificarTarefa(a)}
function silenciarOcorrencia(a){silenciados[a.tarefaId]=chaveOcorrencia(a,new Date());salvar(KEY_SILENCED,silenciados);fecharNotificacao(a);encerrarDisparo();toast('Toque parado. O alarme desta data continua registrado.')}
...
function zerarViradaSemana(){const semana=semanaInicioISO(new Date());if(localStorage.getItem(KEY_WEEK)===semana)return false;alarmes=filtrarSemana(alarmes);silenciados={};salvar(KEY_STATE,alarmes);salvar(KEY_SILENCED,silenciados);salvar(KEY_PENDING,ler(KEY_PENDING,[]).filter(p=>naSemanaAtual(p)));localStorage.setItem(KEY_WEEK,semana);expirarAlarmesRemotos();encerrarDisparo();atualizarBotoes();window.dispatchEvent(new CustomEvent('rotina-family-alarm-week-reset',{detail:{semanaInicio:semana}}));return true}
function boot(){zerarViradaSemana();decorarTarefas();const tbody=document.getElementById('tabelaCorpo');if(tbody)new MutationObserver(decorarTarefas).observe(tbody,{childList:true,subtree:true});document.addEventListener('pointerdown',()=>{const a=audio();a.resume?.().catch(()=>{})},{once:true});escutar();setInterval(verificarDisparo,1000);setInterval(zerarViradaSemana,30000)}
window.addEventListener('online',()=>{sincronizarPendente();expirarAlarmesRemotos()});document.addEventListener('visibilitychange',()=>{if(!document.hidden){zerarViradaSemana();expirarAlarmesRemotos();decorarTarefas();verificarDisparo()}});
