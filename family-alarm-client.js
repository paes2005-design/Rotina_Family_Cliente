...
import {chaveOcorrencia,deveDispararAgora,descreverProximaOcorrencia} from './alarm-schedule-core.js?v=2';
...
async function notificarTarefa(a){if(!('Notification'in window)||Notification.permission!=='granted'||!('serviceWorker'in navigator))return;try{const reg=await navigator.serviceWorker.ready;await reg.showNotification(`⏰ Hora da tarefa: ${a.nomeTarefa||'Tarefa'}`,{body:`Programada para ${a.horaSugeridaInicio||'agora'}. Toque para abrir o Rotina Family.`,icon:'./icon-cliente-192.png',badge:'./icon-cliente-192.png',tag:tagNotificacao(a),renotify:true,requireInteraction:true,data:{url:'./'}})}catch{}}
...
function abrirPainel(tarefa){
  ...
  m.querySelector('#alarmToggle').onclick=()=>{const ativar=!a?.ativo;if(ativar)prepararNotificacoes();m.remove();toast(ativar?'Alarme programado.':'Alarme retirado.');gravar(tarefa,ativar,'CLIENTE').then(ok=>{if(!ok)toast('Não foi possível alterar este alarme.')})};
