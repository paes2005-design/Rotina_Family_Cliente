...
async function queryString(env, collectionId, field, value, now = new Date(), limit = 100) {
  const response = await requestFirestore(env, `${firestoreBase(env)}:runQuery`, {
    method: 'POST',
...
async function getDoc(env, name, now = new Date()) {
  const response = await requestFirestore(env, `https://firestore.googleapis.com/v1/${name}`, {}, now);
...
async function patchDoc(env, name, patch, now = new Date()) {
  ...
  const response = await requestFirestore(env, url.toString(), {
    method: 'PATCH',
    headers: { 'content-type': 'application/json' },
...
async function deleteDoc(env, name, now = new Date()) {
  const response = await requestFirestore(env, `https://firestore.googleapis.com/v1/${name}`, { method: 'DELETE' }, now);
...
async function requireMaster(request, env, now = new Date()) {
...
function groupIdOf(data = {}) {
  return String(data.codigoCliente || data.grupoId || '').trim().toUpperCase();
}

async function groupAdmins(env, groupId, now = new Date()) {
  const first = await queryString(env, 'administradores', 'codigoCliente', groupId, now, 50);
  const second = await queryString(env, 'administradores', 'grupoId', groupId, now, 50);
  return [...new Map([...first, ...second].map(item => [item.name, item])).values()];
}

function dateMs(value, fallback = Number.MAX_SAFE_INTEGER) {
  const parsed = Date.parse(String(value || ''));
  return Number.isFinite(parsed) ? parsed : fallback;
}

function chooseOwner(admins, env) {
  const usable = admins.filter(item => !isMasterEmail(env, item.data?.email));
  const explicit = usable.find(item => String(item.data?.tipoAcesso || '').toLowerCase() === 'proprietario');
  if (explicit) return explicit;
  return [...usable].sort((a, b) => dateMs(a.data?.criadoEm || a.createTime) - dateMs(b.data?.criadoEm || b.createTime))[0] || null;
}
...
async function registerTrial(request, env, now = new Date()) {
  ...
  const groupId = groupIdOf(administrator.data);
  if (!groupId) throw new Error('Grupo não encontrado.');

  const name = `projects/${env.FIREBASE_PROJECT_ID}/databases/(default)/documents/configGrupos/${groupId}`;
  const current = await getDoc(env, name, now);
  if (current?.data?.grupoConfirmado === true) {
    return { success: true, grupoId: groupId, estado: 'confirmado', confirmado: true };
  }
  if (current && isTrialV2(current.data)) {
    return { success: true, grupoId: groupId, estado: commercialState(current.data, now.getTime()), trialFimEm: current.data.trialFimEm || '' };
  }

  const admins = await groupAdmins(env, groupId, now);
  const owner = chooseOwner(admins, env) || administrator;
  ...
  const startMs = dateMs(startSource, now.getTime());
  const patch = startFamilyTrialPatch(new Date(startMs));
  if (current?.data?.grupoBloqueado === true) {
    patch.grupoBloqueado = true;
    patch.bloqueioManual = true;
  }
  const saved = await upsertConfig(env, groupId, patch, now);
  return {
    success: true,
    grupoId: groupId,
    estado: commercialState(saved?.data || patch, now.getTime()),
    trialInicioEm: patch.trialInicioEm,
    trialFimEm: patch.trialFimEm
  };
}

async function handleMasterGroupMutation(request, env, now = new Date()) {
  const caller = await requireMaster(request, env, now);
  const body = await request.json().catch(() => ({}));
  const groupId = String(body.grupoId || '').trim().toUpperCase();
  if (!groupId) throw new Error('Grupo não informado.');

  let saved;
  if (body.action === 'set-group-blocked') {
    saved = await upsertConfig(env, groupId, familyBlockPatch(body.disabled === true, now.toISOString()), now);
  } else if (body.action === 'confirm-group') {
    saved = await upsertConfig(env, groupId, { ...confirmFamilyPatch(now.toISOString()), confirmadoPorMaster: caller.uid }, now);
  } else {
  ...
  return {
    ...
    grupoId: groupId,
    action: body.action,
    estado: commercialState(saved?.data || {}, now.getTime()),
    grupoBloqueado: saved?.data?.grupoBloqueado === true,
    grupoConfirmado: saved?.data?.grupoConfirmado === true
  };
}

export async function commercialStorageSelfTest(env, now = new Date()) {
  const groupId = `TESTE-SMOKE-${now.getTime()}`;
  const name = `projects/${env.FIREBASE_PROJECT_ID}/databases/(default)/documents/configGrupos/${groupId}`;
  try {
    const expiredStart = new Date(now.getTime() - 16 * 86400000);
    let saved = await upsertConfig(env, groupId, startFamilyTrialPatch(expiredStart), now);
    const expiredState = commercialState(saved?.data || {}, now.getTime());
    if (expiredState !== 'teste-expirado') throw new Error(`Esperado teste-expirado; recebido ${expiredState}`);

    saved = await upsertConfig(env, groupId, confirmFamilyPatch(now.toISOString()), now);
    const confirmedState = commercialState(saved?.data || {}, now.getTime());
    if (confirmedState !== 'confirmado') throw new Error(`Esperado confirmado; recebido ${confirmedState}`);

    saved = await upsertConfig(env, groupId, familyBlockPatch(true, now.toISOString()), now);
    const blockedState = commercialState(saved?.data || {}, now.getTime());
    if (blockedState !== 'bloqueado') throw new Error(`Esperado bloqueado; recebido ${blockedState}`);

    return { ok: true, expiredState, confirmedState, blockedState };
  } finally {
    await deleteDoc(env, name, now).catch(() => {});
  }
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === '/admin-master/tree' && request.method === 'GET') {
        return Response.json({ error: 'Árvore global desativada. Use a lista de grupos e consulte um grupo por vez.' }, { status: 410, headers: cors(request) });
      }
      if (url.pathname === '/admin-master/groups' && request.method === 'POST') {
        return Response.json(await handleMasterGroupMutation(request, env), { headers: cors(request) });
      }
    } catch (error) {
      const message = String(error?.message || error).replace(/\s+/g, ' ').slice(0, 320);
      const status = /Acesso exclusivo/.test(message) ? 403 : 400;
