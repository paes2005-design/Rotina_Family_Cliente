const CACHE_NAME='rotina-family-cliente-v32';
...
self.addEventListener('notificationclick',event=>{event.notification.close();event.waitUntil((async()=>{const janelas=await self.clients.matchAll({type:'window',includeUncontrolled:true});for(const janela of janelas){if('focus'in janela){await janela.focus();return}}if(self.clients.openWindow)await self.clients.openWindow('./')})())});
