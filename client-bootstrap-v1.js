(()=>{
  ...
  if(!window.__rotinaResolveAuthBridge)window.__rotinaResolveAuthBridge=value=>{try{resolveReady?.(value||true)}catch{}};
  ...
  const MODULES=Object.freeze([
    ...
    {name:'firebase-repository',src:'./client-firebase-repository-v1.js?v=20260915.3',type:'module',owner:'Participant Data / Firebase Repository',critical:true},
    {name:'sync-scheduler',src:'./client-sync-scheduler-v1.js?v=20260915.3',type:'classic',owner:'Participant Data / Sync Scheduler',critical:true},
    ...
    {name:'history-reconciler',src:'./client-history-reconciler.js?v=5',type:'module',owner:'History Repair / Gap Reconciliation',critical:false},
    ...
    {name:'runtime-build-info',src:'./runtime-build-info.js?v=20260915.3',type:'classic',owner:'Runtime/Version',critical:false}
  ]);
  ...
  async function start(){log('bootstrap.inicio',{modulos:MODULES.length});const results=await Promise.allSettled(MODULES.map(loadScript));const falhas=results.filter(result=>result.status==='rejected').length;runtime.finishedAt=Date.now();runtime.ready=falhas===0;log('bootstrap.pronto',{falhas,tempoMs:runtime.finishedAt-runtime.startedAt},falhas?'warning':'info');window.dispatchEvent(new CustomEvent('rotina-client-bootstrap-ready',{detail:{version:BOOTSTRAP_VERSION,build:BUILD,falhas}}))}
  start().catch(error=>{runtime.errors.push({module:'bootstrap',critical:true,message:String(error?.message||error)});log('bootstrap.erro',{mensagem:String(error?.message||error)},'error')});
})();