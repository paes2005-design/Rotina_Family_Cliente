...
const fmtPct=v=>Number(Number(v).toFixed(2)).toLocaleString('pt-BR');
...
function abrirAjudaRegra(t){
  ...
  const pct75=e.limite100Seg>0?(e.faixa75Seg/e.limite100Seg)*100:0;
  const pct50=e.limite100Seg>0?(e.faixa50Seg/e.limite100Seg)*100:0;
  const extraTotal=pct75+pct50;
  const fator=Number(e.janelaAdicionalPct??100);
  ...
  m.innerHTML=`<div style="width:min(92vw,430px);background:#fff;border-radius:20px;padding:20px;box-shadow:0 18px 55px rgba(0,0,0,.25);color:#1f2937"><h2 style="margin:0 0 10px">⏱️ Como funciona sua tolerância?</h2><p style="margin:0 0 12px;line-height:1.45">Nesta tarefa você tem <strong>${normal}</strong> de tolerância-base na faixa de <strong>100%</strong>.</p><div style="display:grid;gap:8px"><div style="padding:10px 12px;border-radius:12px;background:#fef9c3">🟡 Quando a tolerância-base termina, começa a faixa de <strong>75%</strong> por mais <strong>${faixa75}</strong> (${fmtPct(pct75)}% da tolerância-base).</div><div style="padding:10px 12px;border-radius:12px;background:#ffedd5">🟠 Depois, a faixa de <strong>50%</strong> dura mais <strong>${faixa50}</strong> (${fmtPct(pct50)}% da tolerância-base).</div><div style="padding:10px 12px;border-radius:12px;background:#fee2e2">🔴 Depois de <strong>${maximo}</strong> de atraso total, a tarefa fica na faixa de <strong>0%</strong>.</div></div><p style="margin:12px 0 0;color:#64748b;font-size:13px;line-height:1.4">Este grupo usa ${fmtPct(fator)}% da janela adicional padrão de 25%, equivalente a ${fmtPct(extraTotal)}% da tolerância-base. Esses percentuais representam <strong>tempo</strong>, não desconto de pontos. Nas faixas 100%, 75% e 50%, a tarefa vale todos os pontos cadastrados.</p><p style="margin:8px 0 0;color:#64748b;font-size:13px;line-height:1.4">A tolerância é um saldo único: atraso no início + atraso no término. Começar antes não gasta esse saldo.</p><div style="display:flex;justify-content:flex-end;margin-top:14px"><button type="button" id="clientToleranceHelpClose" class="btn">Entendi</button></div></div>`;
