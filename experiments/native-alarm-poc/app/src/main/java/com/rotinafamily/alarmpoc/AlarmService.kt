...
class AlarmService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        val title = intent?.getStringExtra(AlarmScheduler.EXTRA_TITLE).orEmpty().ifBlank { "Tarefa" }
        val moment = intent?.getStringExtra(AlarmScheduler.EXTRA_MOMENT).orEmpty()
        startForeground(NOTIFICATION_ID, buildNotification(title, moment))
        startAudio()
    ...
    private fun buildNotification(title: String, moment: String): Notification {
        val open = PendingIntent.getActivity(this, 3002, Intent(this, AlarmActivity::class.java).apply {
            putExtra(AlarmScheduler.EXTRA_TITLE, title); putExtra(AlarmScheduler.EXTRA_MOMENT, moment)
        ...
        val prefix = if (moment == "fim") "Fim" else "Início"
        return Notification.Builder(this, CHANNEL_ID)
            .setSmallIcon(android.R.drawable.ic_lock_idle_alarm)
            .setContentTitle("Rotina Family — $prefix")
    ...
    private fun createChannel() {
        val channel = NotificationChannel(CHANNEL_ID, "Alarmes Rotina Family", NotificationManager.IMPORTANCE_HIGH).apply {
            description = "Alarmes agendados do Rotina Family"; setSound(null, null); lockscreenVisibility = Notification.VISIBILITY_PUBLIC
        }
